...
filename = ['C' num2str(n) '/comm_all/newborns.mat'];
if ~exist(filename, 'file')
    simu_one_cycle(n, 0.3)
end
...
adults(1:N, 1) = comm_struct;
for i = 1:2
    load(['HeriData/adults' num2str(I(i))]);
    adults((i-1)*50+1 : i*50) = adults_off(1:50);
end
foldername = ['C' num2str(n+1) '/comm_all'];
if ~exist(foldername,'dir')
    mkdir(foldername)
end
save([foldername '/adults'],'adults');

foldername = 'HeriData2';
if ~isfolder(foldername)
    mkdir(foldername)
end
%%
filename = 'HeriData2/rngn2.mat';
if exist(filename, 'file')
    % set the randome number generator to obtain identical results
    load(filename)
    rng(rngn2)
...
for j = 1:100
    ...
    newborns = comm_all;
    % save the selected communities and the current state of the random number generator
    save([foldername '/newborns' num2str(j)],'newborns');
    parfor rep = 1: min(length(comm_all), N)
        if sum(newborns(rep).M_L) + sum(newborns(rep).H_L) < pcs
            error('An Newborn is empty')
        end
        comm_all(rep) = simu_one_comm(newborns(rep), comm_struct, const_struct);
    ...
    adults_off = comm_all;
    save([foldername '/adults' num2str(j)],'adults_off');
end
