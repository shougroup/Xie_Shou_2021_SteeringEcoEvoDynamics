...
%% Calculate the map if it hasn't been done. Otherwise just load the data in LocalMap.mat. 
if ~exist('LocalMap.mat')
    ...
    for i = x0 : dx : xt
        ...
        for j = y0:dy:yt
            ...
            [T, X] = ode15s(@(t,x) MHDynamics_NoCost(t,x,para), [0 T0], [N0*j; N0*(1-j); 0; R0; 0],options);
            if ~isreal(X)
...
MfracDiff_adjusted1 = MfracT  - ones(xsize, 1) * (y0 : dy : yt);
MfracDiff_adjusted2 = MfracT * (1-spike_frac) - ones(xsize, 1) * (y0 : dy : yt);
figure(2)
contour((x0:dx:xt), (y0:dy:yt), P', (min(P(:)) : 60 : max(P(:))), 'linewidth', 3); % , (min(P(:)) : 240 : max(P(:)))
...
colormap(c)
hold on
contour((x0:dx:xt),(y0:dy:yt), MfracDiff_adjusted1', [0 0], 'color',[255.0 127.0 42.0]/255, 'linewidth',3)
% contour((x0:dx:xt),(y0:dy:yt), MfracDiff_adjusted2', [0 0], 'color',[0 0.5 0.5] , 'linewidth',3)
hold off
ylabel('phi_M(0)')
xlabel('f_P')
