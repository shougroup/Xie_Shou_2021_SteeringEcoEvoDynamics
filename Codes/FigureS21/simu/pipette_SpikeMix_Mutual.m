...
function [comm_rep, H_isolate_out, M_isolate_out] = pipette_SpikeMix_Mutual(comm_selected, newborn_struct, ...
...
B0 = comm_selected.B0(1:H_counter);
all_L = [M_L; H_L];
...
if sum(num_cells) > M_counter+H_counter
    ...
    H_isolate_out.B0 = -ones(spike_clone_num, 1);
    ...
    if ~isempty(H_idx)
        ...
        H_isolate_out.B0(1:t_l) = all_KB_rand(H_idx);
...
for i = 1 : sum(rep_num_M)
    ...
    if H_num >= 1
        ...
        comm_rep(i).B0(1:H_num) = KB_temp(fp_temp<-0.1);
    ...
    if HM_ind(i) > pcs
        if HM_spike(i) >= 1
            ...
            comm_rep(i).B0(1+H_num : HM_spike(i)+H_num) = B0_spike(spike_idx);
        end
    elseif HM_ind(i) < -pcs
        if HM_spike(i) >= 1
